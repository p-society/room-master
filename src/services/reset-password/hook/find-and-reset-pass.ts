...
export default (options = {}): Hook => {
  return async (context: HookContext): Promise<any> => {
    ...
    if (!email || !newPassword || !otp)
      throw new Error('All credentials not provided!');
    ...
    if (resetToken.length === 0)
      throw new Error('No Request for password reset found');
    ...
    await Promise.all(
      resetToken.map((each: { _id: any }) =>
        app.service('reset-password')._remove(each._id)
      )
    );
    context.result = {
